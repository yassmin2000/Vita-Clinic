...
const ActionsCell = ({ row }: { row: Row<LaboratoryTest> }) => {
  ...
  const { mutate: deleteLaboratoryTest, isPending } = useMutation({
    mutationFn: async () => {
      return await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/settings/laboratory-tests/${row.original.id}`,
        {
          headers: {
    ...
    onError: (error) => {
      ...
      return toast({
        title: 'Failed to delete laboratory test',
        description:
          axiosError?.response?.data.message ||
          'Laboratory test could not be deleted, please try again.',
        variant: 'destructive',
      });
    },
    onSuccess: () => {
      ...
      return toast({
        title: 'Laboratory test deleted successfully',
